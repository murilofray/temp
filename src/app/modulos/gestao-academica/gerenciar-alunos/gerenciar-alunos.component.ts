...
import { FormBuilder, FormsModule, ReactiveFormsModule, Validators, FormGroup, Form } from '@angular/forms';
...
export class GerenciarAlunosComponent {
    ...
    if (cepControl) {
      ...
      if (cep.length === 8) {
        this.viaCepService.validarCep(cep).subscribe(
          (resposta) => {
            if (!resposta.erro) {
              // Preenche os campos automaticamente se o CEP for válido
        ...
        this.messageService.add({
          severity: 'error',
          summary: 'Erro',
          detail: 'CEP inválido. Por favor, insira um CEP com 8 dígitos.',
        });
      }
    }
  }
  ...
  async carregarAlunos() {
    try {
      this.alunos = await this.alunoService.getActiveAlunos();
      this.alunosFiltrados = [...this.alunos]; // Inicializa o filtro com todos os alunos
    } catch (error) {
      this.messageService.add({
        severity: 'error',
        summary: 'Erro',
  ...
  async cadastrarAluno() {
    ...
    if (this.alunoForm.valid) {
      ...
      const validationResult = this.dataNascimentoValidator()(dataNascimentoControl);
      ...
      const cpf = this.alunoForm.value.cpf;
      const cpfAlterado = cpf.replace(/\D/g, '');
      ...
      const resposta = await this.alunoService.criarAluno(nome, dataNascimento, nomeMae, celularFormatado, ra, cpfAlterado, sexo, raca.name, beneficiario, logradouro, cep, numero, bairro, cidade, uf, alergiaIds, documentos);
  ...
  async deletarAluno() {
    const resposta = await this.alunoService.deletarAluno(this.alunoSelecionado.id, new Date());
    if (resposta.error) {
  ...
  //não pode ser um valor no futuro  
  ...
  filterAlunos(event: Event) {
    ...
    if (this.isRA(inputValue)) {
      this.alunoService.getByRA(inputValue)
        .then(aluno => {
      ...
      this.alunoService.getByNome(inputValue)
        .then(alunos => {
  ...
  isRA(value: string): boolean {
    const raRegex = /^[\d.-]+$/;
    return raRegex.test(value);
  }
