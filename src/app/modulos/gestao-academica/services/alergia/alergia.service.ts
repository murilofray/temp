...
export class AlergiaService {
  ...
  async downloadRelatorioAlergia(tipoAlergiaId: number) {
    try {
      const query = tipoAlergiaId == -1 || !tipoAlergiaId ? '' : '?tipoAlergiaId=' + tipoAlergiaId;
  ...
  async printRelatorioAlergia(tipoAlergiaId: number) {
    try {
      const query = tipoAlergiaId == -1 || !tipoAlergiaId ? '' : '?tipoAlergiaId=' + tipoAlergiaId;
