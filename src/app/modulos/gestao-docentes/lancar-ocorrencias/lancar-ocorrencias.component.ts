...
export class LancarOcorrenciasComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (tokenJWT) {
      ...
      this.servidorLogadoId = decodedToken.id;
      this.servidorLogadoEscola = decodedToken.escolaId;
    }
  ...
  async enviarOcorrencia() {
    if (this.novaOcorrencia.professor) {
      ...
      try {
        const resposta = await this.ocorrenciaService.criarOcorrencia(
          ocorrencia.servidorId,
          ocorrencia.abonoId,
          ocorrencia.lancadoPor,
          'Não Justificada',
