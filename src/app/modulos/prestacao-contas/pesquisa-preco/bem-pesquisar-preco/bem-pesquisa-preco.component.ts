...
export class BemPesquisaPrecoComponent implements OnInit {
  ...
  private async createBem() {
    (this.bem.pesquisaPrecoId = this.pesquisaPreco.id), (this.bem.menorValor = 0);

    try {
      const resposta = await this.bemService.create(this.bem);
      this.messageService.add({
        severity: 'success',
        summary: 'Adicionado',
        life: 3000,
        detail: `${this.bem.descricao} foi adicionado à Pesquisa de Preço${this.tituloPesquisa}`,
      });
    } catch (error) {
      console.error(error);
      this.messageService.add({
        severity: 'error',
        summary: `Erro ao adicionar: ${this.bem.descricao}`,
        life: 7000,
      });
    }
  }

  private async updateBem() {
    try {
      ...
      this.messageService.add({
        ...
        summary: 'Alterado',
        life: 3000,
        detail: `${this.bem.descricao} foi alterado na Pesquisa de Preço${this.tituloPesquisa}`,
      });
    } catch (error) {
      console.error(error);
      this.messageService.add({
        severity: 'error',
        summary: `Erro ao alterar: ${this.bem.descricao}`,
        life: 7000,
      });
    }
  }
  ...
  async submitedBem() {
    ...
    if (!this.bem.quantidade || Number(this.bem.quantidade) <= 0) {
    ...
    if (!this.isEditMode) await this.createBem();
    else await this.updateBem();
  ...
  private async todosValoresProponentesPreenchidos() {
    ...
    const okProponentes =
      ...
      this.listaBens.every((b: Bem) => {
        return (
          Array.isArray(b.PropostaBem) &&
          b.PropostaBem.length === 3 &&
          b.PropostaBem.every((p: PropostaBem) => p.valor != 0)
        );
      });
