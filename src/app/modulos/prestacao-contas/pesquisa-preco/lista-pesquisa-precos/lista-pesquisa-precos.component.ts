...
export class ListaPesquisaPrecosComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (dataPrestacao) {
      const data = JSON.parse(dataPrestacao);
      this.prestacaoContas = await this.prestacaoContasService.getById(Number(data.id));
      this.buscarTodas(this.prestacaoContas.id);
    } else {
      //   this.prestacaoContas = await this.prestacaoContasService.getById(Number(1));
      //   console.error(`Não existe prestação de contas no LS:\n ${dataPrestacao}`);
  ...
  async salvarPesquisa() {
    this.createPesquisaPreco.titulo = this.tituloPesquisa;
    try {
      ...
      if (resposta) {
        ...
        localStorage.setItem('pesquisaPreco', JSON.stringify(resposta.data as PesquisaPreco));
