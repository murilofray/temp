...
export class ListarMovimentacaoComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (dataContaBancaria && this.anoFiscal) {
      const dadoContaBancaria = JSON.parse(dataContaBancaria);
      const contaBancariaDados = await this.contaBancariaService.getById(Number(dadoContaBancaria.id));
      this.contaBancaria = contaBancariaDados.data as ContaBancaria;
  ...
  async submitMovFinanceira() {
    ...
    if (!this.movimentacaoFinanceira.tipo) {
  ...
  private async buscarTodas(idContaBancaria: number, anoFiscal?: number) {
    ...
    try {
      const resposta = await this.movimentacaoFinanceiraService.getByContaBancaria(idContaBancaria, ano);
      this.listaMovimentacoes = resposta.data;
