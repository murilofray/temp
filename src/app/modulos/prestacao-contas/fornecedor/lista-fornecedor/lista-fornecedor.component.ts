...
export class ListaFornecedorComponent implements OnInit {
  ...
  async coletarDadosFornecedors() {
    ...
    if (!this.fornecedorDOC && !this.isViewMode) {
  ...
  viewFornecedor(fornecedor: any) {
    ...
    this.fornecedor.telefone = this.formatarTelefone(this.fornecedor.telefone);
    if (fornecedor.cpf) this.fornecedorDOC = this.fornecedor.cpf;
    else this.fornecedorDOC = this.fornecedor.cnpj;
